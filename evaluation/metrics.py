...
def lpips_score(img1: np.ndarray, img2: np.ndarray) -> float:
    """
    LPIPS perceptual distance between two images (lower is better).
    ...
    # LPIPS expects (1, 3, H, W) tensors in [-1, 1]
    t1 = torch.from_numpy(img1).permute(2, 0, 1).unsqueeze(0) * 2 - 1
    t2 = torch.from_numpy(img2).permute(2, 0, 1).unsqueeze(0) * 2 - 1

    loss_fn = _lpips_mod.LPIPS(net="alex", verbose=False)
    with torch.no_grad():
        score = loss_fn(t1, t2)
...
def evaluate(model: torch.nn.Module, ref_image: np.ndarray, device: str = "cpu") -> dict:
    """
    Reconstruct image and compute PSNR + SSIM + LPIPS against reference.
    ...
    Returns:
        dict with keys: psnr, ssim, lpips, reconstructed (np.ndarray)
    """
    ...
    return {
        ...
        "lpips": lpips_score(ref_image, recon),
        "reconstructed": recon,
    }
